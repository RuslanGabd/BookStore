...
SELECT p.model, l.price AS price FROM laptop l
JOIN product p ON p.model=l.model
WHERE p.maker = 'B'
UNION
SELECT p.model, pc.price AS price FROM pc 
JOIN product p ON p.model=pc.model
WHERE p.maker = 'B'
UNION
SELECT p.model, prin.price AS price FROM printer prin
JOIN product p ON p.model=prin.model
WHERE p.maker = 'B';
...
SELECT avg(pc.speed) FROM product p
RIGHT JOIN pc ON pc.model=p.model 
WHERE p.maker='A';
...
SELECT p.maker, pch.price  FROM product p
JOIN  
(SELECT model, min(price) AS price FROM printer WHERE color = 'Y') pch 
ON p.model=pch.model;
...
WHERE pc.speed>750 AND maker IN (SELECT maker FROM product pr
JOIN laptop l ON l.model= pr.model
WHERE l.speed>750);

-- task 24
SELECT model FROM
(SELECT model, max(price) FROM pc
union 
SELECT model, max(price) FROM laptop
union
SELECT model, max(price) FROM printer) AS List;
